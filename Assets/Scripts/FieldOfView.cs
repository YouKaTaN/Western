...
public class FieldOfView : MonoBehaviour
{
	...
	[SerializeField] private float maskCutawayDst = 0.1f;
    ...
    {
        StartCoroutine(FindingTargetIE(0.1f));
    }
	...
	private IEnumerator FindingTargetIE(float delay)
	{
		while (true)
		{
            ...
            for (int i = 0; i < targetsInViewRadius.Length; i++)
            {
                ...
                if (Vector3.Angle(transform.forward, dirToTarget) < viewAngle / 2)
                {
                    ...
                    if (!Physics.Raycast(transform.position, dirToTarget, dstToTarget, targetMask))
                    {
						...
						{
							if (player.GetStatusHidden() == false)
							{
								player.Detected();
                                botNavigator.FireToTarget();
                            }
                        }
                    }
                }
